...
object OrderCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //广播ip规则
    val broadcastRef: Broadcast[Array[(Long, Long, String)]] =IPUtils.broadcastIpRules(ssc.sparkContext,args(0))

    //指定消费的 topic 名字
    val topic = "orders"
    //指定kafka的broker地址(sparkStream的Task直连到kafka的分区上，用更加底层的API消费，效率更高)
    val brokerList = "myspark01:9092,myspark02:9092,myspark03:9092"
    ...
    //如果你调用了DStream的transformation，就不能使用直连方式 就不能使用kafkaRDD.asInstanceOf[HasOffsetRanges].offsetRanges
    //kafkaStream.map(_._2).flatMap(_.split(" ")).map((_,1)).updateStateByKey()
